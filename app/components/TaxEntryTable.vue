...
<template>
  <div class="space-y-2">
    <div
      ...
      class="bg-surface rounded-xl border px-4 py-3 transition-colors"
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-x-3 gap-y-2">
        <div v-for="col in dataColumns" :key="col.key" class="space-y-1">
          ...
          <input
            ...
            class="w-full h-9 px-2.5 rounded-lg border border-input bg-background text-body tabular-nums focus:outline-none focus:ring-2 focus:ring-ring focus:border-primary transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            @input="onInput(entry, col, $event)"
          >
          <p v-else class="text-body font-medium tabular-nums text-foreground py-1.5">
